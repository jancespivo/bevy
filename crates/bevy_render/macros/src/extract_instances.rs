...
pub fn derive_extract_instance(input: TokenStream) -> TokenStream {
    ...
    let filter = if let Some(attr) = ast
        .attrs
        .iter()
        .find(|a| a.path().is_ident("extract_instance_filter"))
    {
    ...
    TokenStream::from(quote! {
        impl #impl_generics #bevy_render_path::extract_instances::ExtractInstance for #struct_name #type_generics #where_clause {
            type Query = &'static Self;

            type Filter = #filter;

            fn extract(item: #bevy_ecs_path::query::QueryItem<'_, Self::Query>) -> Option<Self> {
                Some(item.clone())
            }
        }
    })
